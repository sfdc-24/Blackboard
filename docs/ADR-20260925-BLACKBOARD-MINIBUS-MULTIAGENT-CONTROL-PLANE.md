...
by `exact_head` / `reviewed_head`, comparing the first 7 characters of the
head; and writes the latest GO or NO-GO per head into the record's verdict and
row fields. The record for the r5d release path (`r.r5d`) takes only rows whose
id contains `R5D`, and every other record ignores those rows, so the #272
record cites #272's own review rather than the release-plan ruling on the same
head. A head with none is PENDING and keeps its review-request row. The script
also lists verdict rows for newer heads of the same pull requests; it does not
add them, so they are added to the records by hand before the build. The PDF
...
- `r.site222b` Site #222 || Site #222 || ee80098 || GO || CODEX-SITE222-EE80098-GO-20260926T033950Z || - | Source: verdict and row: tools/architecture_verdicts.py at the evidence cutoff
...
- `r.pr272b` #272 || Blackboard #272 || 29f0add || GO || CODEX-PR272-29F0ADD-GO-20260926T0347Z || - | Source: verdict and row: tools/architecture_verdicts.py at the evidence cutoff
- `r.r5d` r5d || The r5d path (0895605 plus only #272) || 8a9349e || NO-GO || CODEX-R5D-8A9349E-NOGO-20260926T0231Z || Its validation image is do not promote; r5b stays at 100%. | Source: verdict and row: tools/architecture_verdicts.py at the evidence cutoff; subject: row CCC-R5D-BUILDER-HOTFIX-RELEASE-PLAN-20260926T0233Z (r5d = the accepted live source 0895605 plus only #272); note: rows CODEX-PR274-31DBBBB-ARCH-NOGO-20260926T0306Z (finding 1), CODEX-R5D-8A9349E-NOGO-20260926T0231Z (runtime), CCC-R5D-NOGO-ACK-20260926T0238Z
...
- `r.pr260b` #260 || #260 client workspaces, Gate 1 || cfdebac || PENDING || CCC-GATE1-REVIEW-REQUEST-PR260-CFDEBAC-20260926T0318Z || - | Source: verdict and row: tools/architecture_verdicts.py at the evidence cutoff; description: Blackboard #260 title
- `p1.prod` R5b serves 100%; rollback r5-0895605-g; r5c at 0% | Source: row CCC-ARCH-E2-STATE-20260926T0349Z (traffic map); FL (r5-0895605-g is the rollback)
...
- `p1.legend` LIVE serving; CURRENT present; PARTIAL bounded proof; GO passed exact head; NO-GO rejected exact head; DARK merged source, switch off; HELD gated; TARGET future. Foundry excluded; Meta optional/off. | Source: `docs/SFDC24-BLACKBOARD-ARCHITECTURE-20260925.pdf` legend; NO-GO and DARK defined by this addendum
...
- `c.controller` PARTIAL || Studio Controller turn and task arbiter || r5b-0895605 || Now: r5b at 100%; #272 merged 03:47Z, not deployed. || Definite-error fallback, provider circuit state, stale-turn fences, hard deadlines, bounded response bodies, receipts. || G2 || G2, r.pr272, r.pr272b | Source: row CCC-ARCH-E2-STATE-20260926T0349Z (r5b at 100%, #272 merged 03:47:27Z, no #272 revision in the traffic map); this ADR, G2
