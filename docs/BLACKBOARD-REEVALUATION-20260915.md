> **ACCEPTED IN DIRECTION, 2026-09-23, WITH ONE CARVE-OUT.**
>
> Mr Salam accepted this document's direction — rules decide, models do, people
> approve — and separately ruled on the rule engine itself:
>
> > *"I'm not sure if Clips will be worth the effort yet; so don't work it into
> > the plans just have it as an option that we can test with later."*
>
> **So CLIPS is out of the plan.** Section 4.1 below argues for it and then
> concedes codex's point that a plain Python policy table gives the same
> explanation traces. That concession is now the instruction: **build the policy
> table, on incident fixtures, and leave `clipspy` as an option to test against
> those same fixtures later.** Nothing downstream should assume a rule engine
> dependency.
>
> **One more thing this document does not know.** It describes the fleet of 15
> September. Since then grok has run out of usage allowance, codex is moving to
> OpenAI cloud, and all nine unattended tasks still run on the laptop. The rule
> service has to be specified against today's fleet, not that one.
>
> Landed from PR #112 rather than merged — the branch had drifted nine days and
> would not rebase. Content is unchanged below this header.
