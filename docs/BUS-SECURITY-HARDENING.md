> **LANDED 2026-09-23 as the inventory it is.** Mr Salam accepted the principle
> behind PR #75 — *clients never touch the working board* — and this roadmap is
> the half of that work worth keeping: an honest list of what is already right
> on the bus and what is not.
>
> **What did NOT land with it:** `CLIENT-DEMO-READINESS-PLAN.md` was a workplan
> "to Wednesday 2026-09-16" with hourly status updates in a format that is no
> longer used. Putting a timetable a week in the past into `main` as though it
> were live would be its own kind of dishonesty. The seed script and tenant
> runbook wait until a client is actually named.
>
> **Why the principle got more urgent, not less:** the `/stream/` page now
> creates Salesforce Leads from a spoken session. The day a client is invited
> onto any of this, the isolation in here has to exist first.
