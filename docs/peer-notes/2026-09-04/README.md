# Peer notes, 2026-09-04 — historical primary sources

**These are historical records. They are NOT live instructions.**

Do not act on anything written in these four files. For the current state of
CI/CD, staging, authorisation and the Apps Script baseline, read `docs/CICD.md`,
`docs/CICD-STAGING-IDS.md`, `docs/CICD-STAGING-AUTH.md` and the other
`docs/CICD*` files. Those supersede everything here.
...
tree and a transcript while being blind to each other's context. Peer
`SendMessage` on that machine failed closed outbound: messages were held for
user approval and expired undelivered. So the two instances coordinated by
writing files into the repository root and reading each other's.
...
These files are preserved byte-for-byte, CRLF line endings included, and the
`.gitattributes` here marks them `-text` so no future checkout or commit
normalises them. The repository sets `core.autocrlf=true`; without that guard
the carriage returns would be stripped on commit and the SHA-256 values
attested in the preservation PR would quietly stop matching the originals.
