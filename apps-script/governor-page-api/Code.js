...
function SYSTEM_PROMPT_(who) {
  ...
  return [
    ...
    "- Emails into Salesforce: the Outlook integration and the Gmail integration let users log emails to records from their inbox. Einstein Activity Capture syncs emails and events automatically, but what it captures is not stored as standard activity records, so reporting on it is limited. Email to Salesforce gives each user a personal address to BCC or forward emails to, which logs them against matching records.",
    "",
