...
MUTANTS = [
    ...
    ("tree: no input cap", [(PAGE, '    text, builder.stopped = _cut(html or "", MAX_HTML_CHARS)',
                             '    text, builder.stopped = html or "", False')]),
    ...
    ("guards: an expired advise lease still counted (Codex, ecee267)", [("cloud/studio-controller/app/guards.py",
        "if isinstance(l, dict) and float(l.get(\"until\") or 0) > now}\n            if leases:",
        "if isinstance(l, dict)}\n            if leases:")]),
    ...
    ("guards: renew brings a replaced lease back (Codex, 59ed871)", [("cloud/studio-controller/app/guards.py",
        ...
        "            if not isinstance(held, dict) or int(held.get(\"fence\") or 0) != int(fence or 0):\n"
        "                leases[owner] = held = {\"fence\": fence, \"until\": now + 1}\n"
        "            if float(held.get(\"until\") or 0) <= now:")]),
    ...
    ("fetch: the stop never reaches the keeper (Codex, 80dfc8f)", [(MAIN,
        "keep_fetch_leases(tenant, claim, still_running, stop_it, deadline)",
        "keep_fetch_leases(tenant, claim, still_running, None, deadline)")]),
    ("fetch: a stopped load still connects (Codex, 80dfc8f; with the gate, ecee267)", [(PAGE,
        "    _stopped(cancel, deadline)               # every step checks: a stopped load goes no further\n", ""), (PAGE,
        "        _stopped(cancel, deadline)\n        response = client.send(", "        response = client.send("), (PAGE,
    ...
    # -- round 12 (Codex Gate 1 NO-GO on ecee267, 02:15Z) -----------------------------------------
    ("tree: addresses found only after the strip (Codex, ecee267)", [(PAGE,
        "    if redacted:\n        value = redact(value)\n", "")]),
    ("tree: a cap keeps the token it cuts (Codex, ecee267)", [(PAGE,
        "    head = text[:n]\n    if not text[n].isspace():", "    head = text[:n]\n    if False:")]),
    ("tree: the attribute limit without the cut rule (Codex, ecee267)", [(PAGE,
        "        a = {k: _cut(v or \"\", 400)[0] for k, v in attrs[:64]",
        "        a = {k: (v or \"\")[:400] for k, v in attrs[:64]")]),
    ("tree: the words budget without the cut rule (Codex, ecee267)", [(PAGE,
        "            piece, over = _cut(data, budget)", "            piece, over = data[:budget], False")]),
    ("tree: a split run read in pieces (Codex, ecee267)", [(PAGE,
        "            self.pending_len += min(len(data), room)\n",
        "            self.pending_len += min(len(data), room)\n        self._flush_data()\n")]),
    ("tree: inline tags end a run (Codex, ecee267)", [(PAGE,
        "        if tag not in _INLINE:\n            self._flush_data()\n        if self.skip:\n            if tag == self.skip_tag:\n"
        "                self.skip += 1",
        "        self._flush_data()\n        if self.skip:\n            if tag == self.skip_tag:\n                self.skip += 1")]),
    ("tree: a parse stopped inside a run keeps its last token (Codex, ecee267)", [(PAGE,
        "        self._flush_data(cut_short=self.stopped)", "        self._flush_data()")]),
    ("guards: an expired lease is renewed (Codex, ecee267)", [("cloud/studio-controller/app/guards.py",
        "            if float(held.get(\"until\") or 0) <= now:\n                return False, False\n", "")]),
    ("fetch: the work never reads its deadline (Codex, ecee267)", [(PAGE,
        "    if deadline is not None and deadline.passed():\n        raise FetchCancelled(\"the load's lease ran out\")",
        "    pass")]),
    ("fetch: an unconfirmed renewal moves the work's deadline (Codex, ecee267)", [(MAIN,
        "                    kept = None                           # unknown: the lease keeps its last expiry\n",
        "                    kept = None                           # unknown: the lease keeps its last expiry\n"
        "                    if deadline is not None:\n"
        "                        deadline.extend(started + FETCH_LEASE_SECONDS - FETCH_WORK_MARGIN_SECONDS)\n")]),
    ("fetch: no gate inside the transport (Codex, ecee267)", [(PAGE,
        "            stop()\n            return self.inner.handle_request(request)",
        "            return self.inner.handle_request(request)")]),
    ("advise: the lease and its use as two writes (Codex, ecee267)", [("cloud/studio-controller/app/guards.py",
        "            counts[lane] = used + 1\n            state.setdefault(\"last\", {})[lane] = now\n"
        "            return (fence, \"\"), True\n        return self._change(name, change)",
        "            return (fence, \"\"), True\n        result = self._change(name, change)\n"
        "        if result[0] is not None:\n            self.take(name, lane, cap)\n        return result")]),
    ("guards: an unwritten release is forgotten (Codex, ecee267)", [("cloud/studio-controller/app/guards.py",
        "                self._owed[key] = fence\n", "                pass\n")]),
    ("guards: an owed release is not settled before the next acquire (Codex, ecee267)", [
        ("cloud/studio-controller/app/guards.py", "        without its lease.\"\"\"\n        self._settle(name)\n",
         "        without its lease.\"\"\"\n")]),
    ("guards: no background retry of an owed release (Codex, ecee267)", [("cloud/studio-controller/app/guards.py",
        "            if start:\n                threading.Thread(", "            if False:\n                threading.Thread(")]),
    ("advise: a cancelled request keeps its lease (Codex, ecee267)", [(MAIN,
