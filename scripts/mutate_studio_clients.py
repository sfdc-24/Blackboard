...
MUTANTS = [
    ...
    ("tree: dotless email hosts kept", [(PAGE, "        if at > start and core > at:\n",
                                         "        if at > start and core > at and \".\" in text[at:core]:\n")]),
    ("tree: a bracketed literal must close within 100 (the old bracket window)", [(PAGE,
        "        if at > start and core > at:\n",
        "        if at > start and core > at and not (text[at + 1:at + 2] == \"[\" and core - at > 102):\n")]),
    ("tree: the old windows, 64 before and 255 after the @", [(PAGE,
        "            yield start, core + 1, EMAIL\n", "            yield max(start, at - 64), min(core + 1, at + 256), EMAIL\n")]),
    ...
    ("exhaust: the outcome is not made durable", [(CORE,
        "        self._record_outcome(session_id, outcome)   # if even that fails", "        (session_id, outcome)   # if even that fails")]),
