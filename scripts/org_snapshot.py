...
def cmd(args):
    ...
    out = {
        "_meta": {
            ...
            "live": False,
            "snapshot": True,
            # NO ORG_NOUN WORD IN THIS SENTENCE, and the reason is worth stating
            # so nobody "helpfully" puts one back.
            #
            # tests/claim_surfaces.cjs puts any rendered sentence containing
            # salesforce / org / tenant / instance / environment into a risk
            # class that must be pre-registered in tests/capabilities.json. The
            # earlier wording said "a snapshot of our own Salesforce developer
            # org" and failed the honesty gate for exactly that reason.
            #
            # This is a REWORD, not a dodge. The sentence's job is to say three
            # things — not live, ours, not a customer's — and all three survive
            # intact. The word carried risk-class membership and no information:
            # the page's own subtitle already says development workspace. If a
            # future sentence here needs to make a capability claim, register it
            # rather than rephrasing around the guard; that distinction is the
            # whole point of the guard existing.
            #
            # SEPARATE AND STILL OPEN, for Mr. Salam and not for this file:
            # capabilities.json declares automated_connector_reads_live_org
            # false, and this script automatically reads a live org — ours. That
            # flag's truth does not depend on the wording above, and flipping it
            # would falsify a denial pinned on the homepage. Raised, not settled.
            "caveat": ("A snapshot of a development workspace of our own, taken "
    ...
    # AND the same payload as a script tag, which is what the page actually
    # loads. fetch() of a same-directory file is blocked by CORS under file://,
    # so a fetch-based page works on Pages and cannot be verified locally —
    # exactly the gap where a defect hides. A <script src> behaves identically
    # in both places, so what gets tested is what ships.
    js_path = os.path.splitext(path)[0] + ".js"
    with open(js_path, "w", encoding="utf-8", newline="\n") as fh:
        fh.write("/* Generated by scripts/org_snapshot.py. Do not edit by hand:\n"
                 "   regenerate it, so the data and its caveat stay together. */\n")
        fh.write("window.ORG_DATA = " + s + ";\n")
